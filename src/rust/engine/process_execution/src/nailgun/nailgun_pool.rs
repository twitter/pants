...
pub struct NailgunPool {
  processes: NailgunProcessMap,
}

impl NailgunPool {
  pub fn new() -> Self {
    NailgunPool {
      processes: NailgunProcessMap::new(),
    }
  }
  // public for tests.
  pub (crate) fn materialize_workdir_for_server(
    store: Store,
  ...
  pub fn connect(
    ...
    // Before doing anything to (re-)start a nailgun server materialize the files
    // in its workdir.
