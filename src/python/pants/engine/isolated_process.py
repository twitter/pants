...
def _post_process_execution(output_conversion,
  snapshot_directory,
  *args):
  """A pickleable top-level function to execute a process.
...
class ExecuteProcess(object):
  ...
  def create_in(product_type, input_selectors, input_conversion):
    ...
    func = functools.partial(_setup_process_execution,
      input_conversion)
    func.__name__ = '{}_and_then_execute_process'.format(
      input_conversion.__name__
    )
  ...
  def create_out(product_type, input_selectors, output_conversion):
    func = functools.partial(_post_process_execution,
      output_conversion)
    func.__name__ = 'execute_process_and_then_{}'.format(
      output_conversion.__name__
    )
...
class ExecuteProcessRequest(datatype('ExecuteProcessRequest', ['args', 'env', 'snapshots','directories_to_create'])):
  """Request for execution with args and snapshots to extract."""

def __new__(cls, args, env, snapshots, directories_to_create, **kwargs):
  """

  :param args: Arguments to the process being run.
  :param args: A dictionary of environment variables and values.
  :param snapshot_subjects: Subjects used to request snapshots that will be checked out into the sandbox.
  :param directories_to_create: Directories to ensure exist in the sandbox before execution.
  """
  if not isinstance(args, tuple):
  ...
  if not isinstance(directories_to_create, tuple):
    raise ValueError('directories_to_create must be a tuple.')
  return super(ExecuteProcessRequest, cls).__new__(cls, args, env, snapshots, directories_to_create, **kwargs)
